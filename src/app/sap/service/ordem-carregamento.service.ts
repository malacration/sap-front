...
export class OrdemCarregamentoService {
  ...
  private vendaFuturaAssing(it): OrdemCarregamento {
    ...
    ordemCarregamento.Ord_CRG_LINHACollection = (
      it.ORD_CRG_LINHACollection || []
    ).map((item) =>
  ...
  getEstoqueEmCarregamento(itemCode: string): Observable<number> {
    return this.http.get<number>(
      `${this.url}/estoque-em-carregamento?ItemCode=${itemCode}`
    );
  }
  getNotasByCarregamentos(idCarregamento: number): Observable<DocumentList[]> {
    ...
    return this.http.get<DocumentList[]>(endpoint).pipe(
      map((docs) =>
        docs
          // opcional: instanciar classe se precisar de métodos
          .map((d) => Object.assign(new Document(), d))
      )
    );
  }
}
