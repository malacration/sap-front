...
import {
  Action,
  ActionReturn,
} from '../../../shared/components/action/action.model';
...
import {
  DocumentLines,
  FutureDeliverySales,
} from '../markting/future-delivery-sales';
...
export class VendaFutura {
  ...
  routerLinkPn(): RouteLink {
    return new RouteLink(
      this.U_cardName,
      '/clientes/parceiro-negocio/' + this.U_cardCode
    );
  }
  ...
  getActions(): Action[] {
    return [
      new Action(
        'Abrir',
        new ActionReturn('selected', this),
      ...
      this.isVerEntregas()
        ? new Action(
            'Carregar Entregas',
            new ActionReturn('carregaEntregas', this)
          )
        : new Action(
            'Carregando...',
            new ActionReturn('none', this),
  ...
  get valorEntregue() {
    ...
    if (this.entregas)
      return formatCurrency(
        this.entregas.reduce(
          (acc, value) =>
            value.DocObjectCode == 'oInvoices'
              ? acc.plus(new Big(value.DocTotal))
              : acc.minus(new Big(value.DocTotal)),
          new Big(0)
        ),
        'pt',
        'R$'
...
export class LinhaItem {
  ...
  get quantidadeEntregue() {
    return this.produtoEntregueLoading
      ? 'Carregando...'
      : this.entregue.toString();
  }
}
