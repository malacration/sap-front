...
export class OrdemCarregamentoSelectedComponent implements OnInit, OnChanges {
  ...
  notasEmitida = [
    new Column('NÃºm. da Nota', 'SequenceSerial'),
    new Column('NÃºm. do Documento', 'DocNum'),
    new Column('CÃ³d. Cliente', 'CardCode'),
    new Column('Nome Cliente', 'CardName'),
    new Column('CÃ³d. Item', 'ItemCode'),
  ...
  cancelarOrdem(): void {
    ...
    if (this.alertService.confirm('Tem certeza que deseja cancelar esta Ordem de Carregamento? Isso irÃ¡ desvincular todos os pedidos.')) {
      this.loading = true;
  ...
  iniciarGeracaoNota(): void {
    if (!this.selected || this.selected.pedidosVenda.length === 0) {
      this.alertService.error('Nenhum pedido disponÃ­vel para gerar nota fiscal.');
      return;
    }
  ...
  abrirModalItinerario(): void {
    if (!this.selected || this.selected.pedidosVenda.length === 0) {
      this.alertService.error('Nenhum pedido disponÃ­vel para gerar itinerÃ¡rio.');
      return;
    }
  ...
  salvarLogistica(): void {
    ...
    this.ordemCarregamentoService.atualizarLogistica(this.selected!.DocEntry, dados).subscribe({
      next: () => {
        this.alertService.confirm('Dados de logÃ­stica salvos com sucesso!');
        if (this.selected) {
          this.selected.U_placa = this.placa;
          this.selected.U_motorista = this.nomeMotorista;
      ...
      error: (err) => {
        this.alertService.error('Erro ao salvar logÃ­stica: ' + (err.error?.message || err.message));
        this.loading = false;
      },
      complete: () => {
        this.loading = false;
  ...
  editarOrdem(): void {
    if (this.selected?.U_Status === 'Fechado' || this.selected?.U_Status === 'Cancelado') {
      this.alertService.error('NÃ£o Ã© possÃ­vel editar uma ordem fechada ou cancelada.');
    } else {
  ...
  private validateForm(): boolean {
    ...
    if (!this.placa || !this.nomeMotorista) {
      this.alertService.error('A placa do veÃ­culo e o nome do motorista sÃ£o obrigatÃ³rios.');
      return false;
    }
    return true;
  }
